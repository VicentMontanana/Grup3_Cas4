...
class versio5 implements interficiefactorialpotencia{

	private static int numerodelusuari;

	public static int calculfactorial (ParametresCalculfactorial parameterObject){
		 
		if(numerodemanat(parameterObject.numero)==0)
		return 1;
		
		else{
			int resultat = numerodemanat(parameterObject.numero) * calculfactorial(new ParametresCalculfactorial(numerodemanat(parameterObject.numero)-1));
        	...
        	switch(numerodemanat(opcio)) {
				...
				numerodelusuari = 0;
		    	    ...
		    	    while(numerodemanat(numerodelusuari) < 0){
		   	    	...
		   	    	System.out.println(calculfactorial(new ParametresCalculfactorial(numerodemanat(numerodelusuari))));         		 
